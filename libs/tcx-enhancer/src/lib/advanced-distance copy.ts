...
function getRandomSidelinePosition(): Position {
    ...
    return {
        LatitudeDegrees: lat + metersToLatDegrees(alongSidelineVariation), // TODO always one side
...
function generateMovementDirection(
    ...
    // Reduce center bias significantly and make it more dynamic
    const centerBias = Math.min(
        0.3,
        Math.pow(distanceFromCenter / maxRadius, 1.5)
    ); // Reduced from 2 to 1.5, capped at 0.3
...
export function interpolatePosition(
    ...
    if (speed > 0) {
        ...
        if (newDistanceFromCenter > radius) {
            // Scale back to boundary and generate new target
